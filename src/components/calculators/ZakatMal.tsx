  ...
  return (
    <div>
      ...
      <Tabs value={subTab} onValueChange={(val) => {
        ...
        <TabsList className="grid grid-cols-3 sm:grid-cols-6 w-full mb-4 h-auto gap-1 p-1">
          <TabsTrigger value="emas" className="text-xs sm:text-xs px-1.5 sm:px-2 py-2.5 min-h-[44px] data-[state=active]:shadow-sm">Emas & Perak</TabsTrigger>
          <TabsTrigger value="uang" className="text-xs sm:text-xs px-1.5 sm:px-2 py-2.5 min-h-[44px]">Uang</TabsTrigger>
          <TabsTrigger value="dagang" className="text-xs sm:text-xs px-1.5 sm:px-2 py-2.5 min-h-[44px]">Bisnis</TabsTrigger>
          <TabsTrigger value="tani" className="text-xs sm:text-xs px-1.5 sm:px-2 py-2.5 min-h-[44px]">Pertanian</TabsTrigger>
          <TabsTrigger value="profesi" className="text-xs sm:text-xs px-1.5 sm:px-2 py-2.5 min-h-[44px]">Profesi</TabsTrigger>
          <TabsTrigger value="fitrah" className="text-xs sm:text-xs px-1.5 sm:px-2 py-2.5 min-h-[44px]">Fitrah</TabsTrigger>
        </TabsList>
            ...
            {berat > 0 && (
              <ResultCard title="Hasil Perhitungan">
                ...
                <div className="mt-3 flex gap-2">
                  <ShareButton getText={() => `Zakat ${jenisLogam}: ${berat}g × ${formatIDR(jenisLogam === 'emas' ? hargaEmas : hargaPerak)} = ${formatIDR(emasResult.totalNilai)}. Zakat: ${formatIDR(emasResult.zakatNominal)}`} />
                  {emasResult.wajib && <Button variant="outline" size="sm" className="gap-1.5 text-xs" onClick={() => { saveZakatEntry({ type: jenisLogam, label: `Zakat ${jenisLogam}`, amount: emasResult.zakatNominal, details: `${berat}g × ${formatIDR(jenisLogam === 'emas' ? hargaEmas : hargaPerak)}` }); toast.success('Tersimpan di riwayat'); }}><Save className="h-3.5 w-3.5" />Simpan</Button>}
                </div>
              </ResultCard>
            )}
          </CardContent></Card>
          ...
          <Card><CardContent className="pt-5 space-y-4">
            ...
            {dagangResult.hartaBersih > 0 && (
              <ResultCard title="Hasil Perhitungan">
                ...
                <div className="mt-3 flex gap-2">
                  <ShareButton getText={() => `Zakat Perdagangan: Harta Bersih ${formatIDR(dagangResult.hartaBersih)}. Zakat: ${formatIDR(dagangResult.zakatNominal)}`} />
                  {dagangResult.wajib && <Button variant="outline" size="sm" className="gap-1.5 text-xs" onClick={() => { saveZakatEntry({ type: 'dagang', label: 'Zakat Bisnis', amount: dagangResult.zakatNominal, details: `Harta bersih ${formatIDR(dagangResult.hartaBersih)}` }); toast.success('Tersimpan di riwayat'); }}><Save className="h-3.5 w-3.5" />Simpan</Button>}
                </div>
              </ResultCard>
            )}
          </CardContent></Card>
